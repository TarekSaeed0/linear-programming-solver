...
export class App {
  ...
  onSolve() {
    ...
    const cleanObjective = {
      ...
      coefficients: value.variables_constraints?.map((_, i) =>
        safeParse(value.objective?.coefficients?.[i]),
      ),
    };
    ...
    const cleanConstraints =
      value.constraints?.map((c: any) => ({
        ...
        coefficients: value.variables_constraints?.map((_, i) => safeParse(c?.coefficients?.[i])),
    ...
    const problem = {
      ...
      variables_constraints: value.variables_constraints?.map((v: any) => ({
        type: v.type,
        variable: {
